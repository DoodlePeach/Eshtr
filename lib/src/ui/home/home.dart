...
class _HomeState extends State<Home> with TickerProviderStateMixin {
  ...
  List<Widget> buildLisOfBlocks(BlocksModel snapshot) {
    ...
    if(snapshot.settings.showOnsale == 1) {
      list.add(ProductScroll(products: snapshot.onSale, context: context, title: snapshot.localeText.featured));
    }

    if(snapshot.settings.showFeatured == 1) {
      list.add(ProductScroll(products: snapshot.featured, context: context, title: snapshot.localeText.sale));
    }
  ...
  _scanBarCode() async {
    ...
    if (result.isNotEmpty) {
      showDialog(
          context: context,
          child: FindBarCodeProduct(result: result, context: context));
    }
  }
}
