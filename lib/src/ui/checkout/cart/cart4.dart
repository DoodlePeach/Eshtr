...
class _CartPageState extends State<CartPage> {
  ...
  void initState() {
    ...
    widget.checkoutBloc.checkoutForm.listen((onData) => setAddressCountry(onData));
  }

  List<Widget> _createShoppingCartRows(CartModel shoppingCart) {
    return shoppingCart.cartContents.map((CartContent item) => ShoppingCartRow(product: item)).toList();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: widget.hideArrowButton == true ? AppBar(
  ...
  buildCart(ThemeData localTheme, CartModel shoppingCart) {
    return ListView(
          children: <Widget>[
            ...
            ShoppingCartSummary(
                cart: shoppingCart, model: widget.appStateModel),
              ...
              children: <Widget>[
                Row(
                  children: <Widget>[
                    ...
                    Text(
                      parseHtmlString(cart.cartTotals.subtotal,
                      ),
                      style: smallAmountStyle,
                    ),
                  ],
                ...
                couponsWidgets.length > 0 ? Column(
                  children: [
                    const SizedBox(height: 6.0),
                    Column(
                                ...
                                child: Row(
                                  ...
                                  children: <Widget>[
                                    Container(
                                      ...
                                      height: 30,
                                      decoration: BoxDecoration(
                                          color: Color(0xffafc2cb).withOpacity(.2)
                                      ),
                                      alignment: Alignment.center,
                                      child:InkWell(
                                        onTap: _onDecreaseQty,
                                        child: Icon(Icons.remove,color: iconColor,size: 20,),
                                      ),
                                    ),
                                    isLoading
                                        ? Container(
